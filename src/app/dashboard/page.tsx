...
export default function DashboardPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (planRes.ok) {
          ...
          const days: MealPlanDay[] = planData.mealPlan?.days || [];

          if (days.length > 0) {
            const today = new Date().toISOString().split("T")[0];
            const todayPlan = days.find(
              (d: MealPlanDay) => d.date.split("T")[0] === today
            );
  ...
  const caloriePercent = mealPlanDay
    ? Math.min(Math.round((mealPlanDay.summaryCalories / preferences.dailyCalories) * 100), 100)
    : 0;
  const proteinPercent = mealPlanDay
    ? Math.min(Math.round((mealPlanDay.summaryProtein / preferences.dailyProtein) * 100), 100)
    : 0;
  const fatPercent = mealPlanDay
    ? Math.min(Math.round((mealPlanDay.summaryFat / preferences.dailyFat) * 100), 100)
    : 0;
  const carbsPercent = mealPlanDay
    ? Math.min(Math.round((mealPlanDay.summaryCarbs / preferences.dailyCarbs) * 100), 100)
    : 0;
